...
func on_point_scored(body):
	speed += 15 # speeds up the gamepley of the pipes. Note: make sure to also change the speed of the ground and the pipes spawning.
	point_scored.emit()

func _on_body_entered(body): # ground 1 & 2 connected to this function here.
	bird_crashed.emit()
	stop()
	(body as Bird).stop()
